        ...
        parseDataset ["cat1", "cat2"] <$>
            readRawData "data/categorical.csv" >>= \case
                ...
                Right Dataset { .. } -> do
                    ...
                    colData col1 `shouldBe` [1, 0, 1, 0, 0]
                    let Just col2 = colByName' "cat2_Z"
                    colData col2 `shouldBe` [0, 0, 0, 1, 0]
    describe "parseDataset" $ do
        it "does the thing" $ do
...
spec_readRawData = do
    it "CSV.readRawData should read all columns" $ do
        ...
        dataColumn "cat3" `shouldBe` Left "unknown column '\"cat3\"', available cols=[\"cat1\",\"cat2\"]"
    it "CSV.readRawData' should only read requested columns" $ do
        RawData { .. } <- readRawData' (=="cat1")  "data/categorical.csv"
        names `shouldBe` ["cat1"]
        dataColumn "cat1" `shouldBe` Right ["C", "B", "C", "A", "B"]
        dataColumn "cat2" `shouldBe` Left "unknown column '\"cat2\"', available cols=[\"cat1\"]"
